      ...
      while (remaining.length > 0) {
        ...
        const body =
          inputMode === 'fetch' && batchNum === 1
            ? { inputMode: 'fetch' as const, cutoffDate, dryRun: false, step: 'void' as const, batchLimit: unpayFirst ? 2 : 20, includePaid, unpayFirstBeforeVoid: unpayFirst }
            : { inputMode: 'csv' as const, invoiceNumbers: remaining, dryRun: false, step: 'void' as const, batchLimit: unpayFirst ? 2 : 20, includePaid, unpayFirstBeforeVoid: unpayFirst }
        const res = await fetch('/api/xero/invoice-cleanup', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(body),
